      ...
      <div className="space-y-3 max-h-[400px] overflow-y-auto">
        {loading ? (
          ...
          logs.map((log, index) => {
            ...
            const match = log.log_message.match(
              /transferred (\d+) x (.+?) \((.+?)\) from/,
            );

            return (
              <div
                key={index}
                className="group rounded-lg border border-border bg-muted/30 p-4 transition-colors hover:bg-muted/60"
              >
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    <p className="inline-flex items-center gap-1">
                      ...
                      Transfer Date:
                      <span className="ml-1">
                        {new Date(log.transferred_at).toLocaleDateString()}
                      </span>
                    </p>
                  </div>
                </div>
