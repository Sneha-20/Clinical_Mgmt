...
export default function useTransferProducts() {
  ...
  const removeTempSerial = (sn) =>
    setTempSerials((s) => s.filter((x) => x !== sn));
  ...
  const addProduct = () => {
    ...
    if (item.stock_type === "Serialized") {
      if (!tempSerials || tempSerials.length === 0) {
        showToast({
          type: "error",
          message: "Please add at least one serial number.",
        });
        return;
      }
      setProducts((p) => [
        ...p,
        { item, quantity: 0, selectedSerials: tempSerials },
      ]);
    } else {
  ...
  const handleSubmit = async () => {
    if (!toClinicId) {
      showToast({
        type: "error",
        message:
          "Please select a destination clinic .",
      });
      return;
    }
    ...
    const payload = {
      ...
      products: products.map((p) => {
        if (p.item.stock_type === "Serialized") {
          return {
            source_inventory_id: p.item.id,
            serial_numbers: p.selectedSerials,
          };
        }
    ...
    try {
      ...
      showToast({
        type: "success",
        message: res?.message || "Transfer submitted successfully!",
      });
      setToClinicId("");
      setNotes("");
      setProducts([]);
