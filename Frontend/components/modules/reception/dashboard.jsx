    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3 sm:space-y-4">
          <div className="flex gap-2 items-center relative max-w-max">
            <Input
              placeholder="Search by name or phone..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="flex-1 text-sm  w-[300px] top-[15px] pr-6"
            />
              ...
              <table className="w-full text-xs sm:text-sm min-w-max sm:min-w-0">
                <thead className="border-b border-slate-200 bg-slate-50">
                  <tr className="text-slate-600 text-left">
                    ...
                    <th className="py-2 px-2 sm:px-3 font-medium hidden sm:table-cell">
                      Phone
                    </th>
                    <th className="py-2 px-2 sm:px-3 font-medium hidden md:table-cell">
                      Purpose of visit
                    </th>
                    <th className="py-2 px-2 sm:px-3 font-medium hidden lg:table-cell">
                      Appointment Date
                    </th>
                    <th className="py-2 px-2 sm:px-3 font-medium">Status</th>
                    <th className="py-2 px-2 sm:px-3 font-medium text-center">
                      Action
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {filteredPatients.map((patient) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-2 sm:px-3 text-slate-600 hidden sm:table-cell">
                        {patient.phone}
                      </td>
                      <td className="py-2 px-2 sm:px-3 text-slate-600 hidden md:table-cell text-xs">
                        {patient.visitType}
                      </td>
                      <td className="py-2 px-2 sm:px-3 text-slate-600 text-xs hidden lg:table-cell">
                        {patient.appointmentDate}
                      </td>
                      <td className="py-2 px-2 sm:px-3">
                        <span className="px-1.5 py-1 bg-teal-100 text-teal-700 rounded-full text-xs font-medium">
                          {patient.status}
                        </span>
                      </td>
