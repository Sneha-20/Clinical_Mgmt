    ...
    <div className="p-4">
      <Card>
        <CardHeader className="pb-3 sm:pb-4">
          ...
          <div className="text-xs sm:text-sm">List of products pending approval</div>
        </CardHeader>
        <div className="overflow-x-auto -mx-3 sm:mx-0">
          {loading ? (
            <div className="text-center py-8 text-slate-500">Loading...</div>
          ) : pendingProducts.length === 0 ? (
            <div className="text-center py-8 text-slate-500">No pending products found</div>
          ) : (
              ...
              <tbody>
                {pendingProducts.map((item) => (
                  <tr key={item.id} className="border-b border-border hover:bg-muted/50">
                    ...
                    <td className="text-left py-2 sm:py-3 px-2 sm:px-3 hidden lg:table-cell">{item.brand_name}</td>
                    <td className="text-left py-2 sm:py-3 px-2 sm:px-3 hidden lg:table-cell">{item.model_type_name}</td>
                    <td className="text-center py-2 sm:py-3 px-2 sm:px-3 font-semibold">{item.quantity_in_stock || 0}</td>
                    <td className="text-center py-2 sm:py-3 px-2 sm:px-3 hidden md:table-cell">₹{parseFloat(item.unit_price || 0).toFixed(2)}</td>
                    <td className="text-center py-2 sm:py-3 px-2 sm:px-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium inline-block ${item.status === "Good" ? "bg-green-100 text-green-600" : item.status === "Low" ? "bg-yellow-100 text-yellow-600" : "bg-red-100 text-red-600"}`}>{item.status}</span>
                    </td>
                    <td className="text-center py-2 sm:py-3 px-2 sm:px-3">
                      <Button onClick={() => approveProduct(item.id)} disabled={approvingId === item.id} className="text-xs">
