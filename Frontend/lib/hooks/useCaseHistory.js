...
export default function useCaseHistory() {
  ...
  const fetchPatientFormData = useCallback(
    async (id) => {
      if (!id) return;
  ...
  }, [searchTerm, fetchTrialDeviceList]);

  const registerTrialForm = useCallback(
    async (data) => {
      dispatch(startLoading());
      try {
  ...
  const registerCasehistory = useCallback(
    async (data) => {
      dispatch(startLoading());
      try {
  ...
  const registerReports = useCallback(
    async (data) => {
      dispatch(startLoading());
      try {
