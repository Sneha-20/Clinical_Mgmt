...
class PartsUsedListView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            # Start with base queryset
            inventory_items = InventoryItem.objects.filter(clinic=self.request.user.clinic, quantity_in_stock__gt=0,stock_type='Non-Serialized').exclude(category='Hearing Aid').order_by('product_name')
